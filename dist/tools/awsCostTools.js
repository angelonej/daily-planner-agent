...
export async function getAwsCostSummary() {
    ...
    if (results?.Groups) {
        for (const g of results.Groups) {
            ...
            if (amt > 0)
                byService.push({ service: svc, amountUSD: amt });
            mtdTotalUSD += amt;
        }
    ...
    try {
        ...
        yesterdayUSD = parseFloat(ydRes.ResultsByTime?.[0]?.Total?.UnblendedCost?.Amount ?? "0");
    }
    catch {
    ...
    try {
        ...
        const forecastAmt = parseFloat(forecastRes.Total?.Amount ?? "-1");
        if (forecastAmt >= 0) {
...
export function formatAwsCostSummary(s) {
