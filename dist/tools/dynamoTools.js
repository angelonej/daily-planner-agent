...
export async function getSettingsFromDynamo(userId = USER_ID) {
...
export async function saveSettingsToDynamo(settings, userId = USER_ID) {
    const client = getClient();
    if (!client)
        return;
    try {
...
const MAX_FACTS = 100;
export async function getMemoryFacts(userId = USER_ID) {
    const client = getClient();
    if (!client)
        return [];
    try {
...
export async function saveMemoryFacts(facts, userId = USER_ID) {
    const client = getClient();
    if (!client)
        return;
...
export async function appendMemoryFacts(newFacts, userId = USER_ID) {
...
export async function removeMemoryFacts(indices, userId = USER_ID) {
    const existing = await getMemoryFacts(userId);
...
export async function clearMemoryFacts(userId = USER_ID) {
