...
function loadSubscriptions(): webpush.PushSubscription[] {
  try {
    if (!fs.existsSync(SUBS_FILE)) return [];
    return JSON.parse(fs.readFileSync(SUBS_FILE, "utf-8")) as webpush.PushSubscription[];
  } catch { return []; }
}

function saveSubscriptions(subs: webpush.PushSubscription[]): void {
  try {
    fs.mkdirSync(path.dirname(SUBS_FILE), { recursive: true });
    fs.writeFileSync(SUBS_FILE, JSON.stringify(subs, null, 2), "utf-8");
  } catch (err) { console.error("push-sub save error:", err); }
}

export function addPushSubscription(sub: webpush.PushSubscription): void {
...
async function checkAwsCostAlert(): Promise<void> {
  const today = new Date().toISOString().slice(0, 10);
  if (lastCostAlertDay === today) return; // only alert once per day
  try {
