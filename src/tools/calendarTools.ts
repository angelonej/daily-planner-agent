...
async function loadCalendarTokens() {
  ...
  if (!fs.existsSync(tokenPath)) {
    throw new Error(
      `No token found for calendar account "${alias}". ` +
...
export interface CreateEventParams {
  title: string;
  startIso: string;   // e.g. "2026-02-25T14:00" or "2026-02-25"
  endIso: string;
  location?: string;
  description?: string;
}
...
async function getAllCalendarIds(calendar: Awaited<ReturnType<typeof buildCalendarClient>>): Promise<{ id: string; primary: boolean }[]> {
  ...
  await Promise.all(
    calendarIds.map(async ({ id }) => {
      try {
        ...
        console.warn(`Skipping calendar ${id}:`, (err as Error).message);
      }
    })
  );
...
// ─── Create a new event ────────────────────────────────────────────────────
export async function createCalendarEvent(params: CreateEventParams): Promise<{ eventId: string; link: string }> {
  const calendar = await buildCalendarClient();
...
export async function updateCalendarEvent(params: UpdateEventParams): Promise<void> {
  ...
  if (params.startIso !== undefined) patch.start = toGoogleDateTime(params.startIso);
  if (params.endIso !== undefined) patch.end = toGoogleDateTime(params.endIso);
  ...
  await Promise.all(
    calendarIds.map(async ({ id }) => {
      try {
        ...
        console.warn(`Skipping calendar ${id} in search:`, (err as Error).message);
      }
    })
  );
