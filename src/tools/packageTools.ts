...
function isArrivingToday(subject: string, snippet: string, emailDate: string): boolean {
  ...
  if (isRecent && DELIVERED_KEYWORDS.some((kw) => combined.includes(kw))) return true;
  // Recent in-transit email
  if (isRecent && (combined.includes("out for delivery") || combined.includes("on its way"))) return true;
  return false;
}

/** Detect likely carrier from email sender/subject when no tracking number is present */
function detectCarrierFromEmail(from: string, subject: string): PackageInfo["carrier"] {
...
function extractTrackingNumbers(
  text: string
): Array<{ carrier: PackageInfo["carrier"]; number: string; url: string }> {
