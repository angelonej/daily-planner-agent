...
import { MorningBriefing, CalendarEvent } from "../types.js";
...
function briefingToHtml(briefing: MorningBriefing): string {
  ...
  const newsHtml =
    briefing.news.length > 0
      ? briefing.news
          .map((n) =>
            `<h4>${n.topic}</h4><ul>${n.articles.slice(0, 3).map((a) => `<li><a href="${a.url}">${a.title}</a></li>`).join("")}</ul>`
          )
          .join("")
      : "<p>No news loaded.</p>";
...
export async function sendWeeklyDigestEmail(
  ...
  if (!to) return { success: false, error: "No recipient. Set DIGEST_EMAIL_TO in .env" };
  ...
  try {
    ...
    const calendarHtml = byDay.size > 0
      ? Array.from(byDay.entries()).map(([day, evs]) =>
          `<h4 style="color:#4a4a6a;margin:16px 0 6px">${day}</h4><ul>${evs.map(e =>
            `<li><b>${e.start}–${e.end}</b>: ${e.title}${e.location ? ` @ ${e.location}` : ""}</li>`
          ).join("")}</ul>`
        ).join("")
    ...
    const taskHtml = openTasks.length > 0
      ? `<ul>${openTasks.slice(0, 15).map((t: any) =>
          `<li>☐ ${t.title}${t.due ? ` <small>(due ${new Date(t.due).toLocaleDateString("en-US", { month: "short", day: "numeric" })})</small>` : ""}</li>`
        ).join("")}</ul>`
      : "<p>No open tasks.</p>";
