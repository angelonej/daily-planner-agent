...
export interface Reminder {
  ...
  active: boolean;
  createdAt: string;
  lastFiredDate?: string;  // ISO date "YYYY-MM-DD" — prevents double-firing same day
}
...
function load(): Reminder[] {
  try {
    ...
    return JSON.parse(fs.readFileSync(DATA_FILE, "utf-8")) as Reminder[];
  } catch { return []; }
}
...
export function addReminder(input: Omit<Reminder, "id" | "createdAt" | "active">): Reminder {
...
export function updateReminder(id: string, changes: Partial<Reminder>): Reminder | null {
  const reminders = load();
...
export function getDueReminders(tz: string): Reminder[] {
  ...
  for (const r of reminders) {
    ...
    if (nowHour !== rH || nowMin !== rM) continue;
