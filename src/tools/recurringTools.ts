...
import { RecurringSuggestion } from "../types.js";
...
function medianTime(times: number[]): string {
 ...
 */
export async function suggestRecurringEvents(
  weeksBack = 4,
  minOccurrences = 2
  ...
  const allEvents: Array<{ title: string; start: Date; end: Date; calendarId: string }> = [];
  ...
  // ─── Group by (normalizedTitle, dayOfWeek) ─────────────────────────────
  type Group = {
    title: string;
    dayOfWeek: number;
    starts: number[];  // ms timestamps
    ends: number[];
  };
  ...
  for (const g of groups.values()) {
    if (g.starts.length < minOccurrences) continue;
    ...
    let confidence: RecurringSuggestion["confidence"];
    if (occ >= 4) confidence = "high";
    else if (occ >= 3) confidence = "medium";
    else confidence = "low";
    ...
    if (g.starts.length >= 2) {
      ...
      if (stdDev > 30 * 60 * 1000) confidence = "low"; // > 30 min variance
    }
  ...
  suggestions.sort((a, b) =>
    confOrder[a.confidence] - confOrder[b.confidence] || b.occurrences - a.occurrences
  );
...
export function formatRecurringSuggestions(suggestions: RecurringSuggestion[]): string {
